            ...
            <div class="panel panel-success">
                    ...
                    <tbody>
                        <c:forEach items="${pageInfo.list}" var="emp">
                            <tr>
                                <td>${emp.trainId}</td>
                                <td>${emp.content}</td>
                                <td><fmt:formatDate pattern="yyyy-MM-dd" value="${emp.time}"/></td>
                                <td>${emp.dept.deptName}</td>
                                <td>${emp.position.positionName}</td>
                                <td>
                                    <a href="#" role="button" id="train_edit" class="btn btn-primary emp_edit_btn" data-toggle="modal" data-target=".train-update-modal"><span class="glyphicon glyphicon-pencil" aria-hidden="true"/>编辑</a>
                ...
                <div class="panel-body">
                    ...
                    <nav aria-label="Page navigation" class="pull-right">
                        <ul class="pagination">
                            <li><a href="gettrainList?pageNo=1">首页</a></li>
                            ...
                            <c:forEach begin="1" end="${pageInfo.pages<5?pageInfo.pages:5}" step="1" var="itemPage">
                                <c:if test="${pageInfo.pageNum == itemPage}">
                                    <li class="active"><a href="gettrainList?pageNo=${itemPage}">${itemPage}</a></li>
                                </c:if>
                                <c:if test="${pageInfo.pageNum != itemPage}">
                                    <li><a href="gettrainList?pageNo=${itemPage}">${itemPage}</a></li>
                                </c:if>
                            </c:forEach>
                            ...
                            <li><a href="gettrainList?pageNo=${pageInfo.pages}">尾页</a></li>
                        </ul>
                    </nav>
                </div>
...
<%@ include file="./trainupdate.jsp"%> 
<%@ include file="./trainAdd.jsp"%> 
